"""
PHASE 4 & 5: STREAMLIT WEB INTERFACE
=====================================
Professional web interface for the chatbot with:
- Clean, modern UI
- Conversation memory
- Source citations
- Chat history
...
class RAGChatbot:
    ...
    def load_vector_database(_self):
        ...
        index_path = os.path.join(_self.db_path, 'index.faiss')
        chunks_path = os.path.join(_self.db_path, 'chunks.pkl')
        
        _self.index = faiss.read_index(index_path)
        
        with open(chunks_path, 'rb') as f:
            _self.chunks = pickle.load(f)
    ...
    def load_embedding_model(_self):
        ...
        import torch
        _self.embedding_model = SentenceTransformer('all-MiniLM-L6-v2', device='cpu')
